...
function preload(){
	...
	entryimg2 = loadImage("entry/1.png");

	dbimg = loadImage("box.png");
...
function setup(){
	...
	db = createSprite(560, 110);
	db.addImage("hfg", dbimg);
	db.scale = 0.3;

	entry2 = createSprite(500, 350);
	entry2.addImage("sss", entryimg2);
	entry2.scale = 0.5;
...
function draw(){
	...
	textStyle("bold");
	fill("#fffdd0 ");
	text("WELCOME DARK WARRIOR", 470, 75);
	text("ARE YOU READY TO FIGHT", 465, 100);
	text("WITH OPPONENT", 495, 125);
	if(entry.y >=340){
		entry.velocityY = 0;
		entry.visible = false;
		entry2.visible = true;
